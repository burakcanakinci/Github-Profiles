  ...
  <div class="main">
    <div class="container-main">
      <div class="header">
        ...
        <!-- Dark Mode Switch -->
        <div v-if="userTheme==='dark-theme'" class="btn">
          <input
            @change="toggleTheme"
            id="checkbox"
            type="checkbox"/>
          ...
          <svg class="light-btn-logo" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Light Mode Switch -->
        <div v-else-if="userTheme==='light-theme'" class="btn-light switch-path-light">
          <input
            @change="toggleTheme"
            id="checkbox"
            type="checkbox"/>
      ...
      <div class="search-bar">
        <div class="search-input">
          <svg class="src" height="24" width="25" xmlns="http://www.w3.org/2000/svg">
            <path class="path-color"
              d="M10.609 0c5.85 0 10.608 4.746 10.608 10.58 0 2.609-.952 5-2.527 6.847l5.112 5.087a.87.87 0 01-1.227 1.233l-5.118-5.093a10.58 10.58 0 01-6.848 2.505C4.759 21.16 0 16.413 0 10.58 0 4.747 4.76 0 10.609 0zm0 1.74c-4.891 0-8.87 3.965-8.87 8.84 0 4.874 3.979 8.84 8.87 8.84a8.855 8.855 0 006.213-2.537l.04-.047a.881.881 0 01.058-.053 8.786 8.786 0 002.558-6.203c0-4.875-3.979-8.84-8.87-8.84z"
              fill="#0079ff"/>
          </svg>
          <input v-model="username" class="width" type="text" placeholder="Search Github username..." />
          <div class="no-result">
            <h4 v-if="error" style="color: #e13a42">No results</h4>
            <h4 v-else-if="!error"></h4>
          </div>
        </div>
        <button @click="getUser()" type="submit" class="src-btn">Search</button>
      </div>
      <div class="container-info">
        ...
        <div class="info">
          ...
          <div>
            ...
            <h4>Joined {{ getDate(user.created_at) }}</h4>
            </div>
            <div class="info-header">
              <h3 class="blue-color">@{{ user.login }}</h3>
              <p v-if="user.bio">{{ user.bio }}</p>
              <h4 v-else class="opacity">This profile has no bio</h4>
            </div>
          </div>
          <!-- Repo Info -->
          <div class="repo">
          ...
          <div class="navigation">
            <div class="top-links">
              <!-- Location -->
              <div v-if="user.location" class="links">
              ...
              <div v-else class="links">
                <svg class="opacity" height="20" width="14" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M12.797 3.425C11.584 1.33 9.427.05 7.03.002a7.483 7.483 0 00-.308 0C4.325.05 2.17 1.33.955 3.425a6.963 6.963 0 00-.09 6.88l4.959 9.077.007.012c.218.38.609.606 1.045.606.437 0 .828-.226 1.046-.606l.007-.012 4.96-9.077a6.963 6.963 0 00-.092-6.88zm-5.92 5.638c-1.552 0-2.813-1.262-2.813-2.813s1.261-2.812 2.812-2.812S9.69 4.699 9.69 6.25 8.427 9.063 6.876 9.063z"
                    fill="#fefefe"/>
                </svg>
                <h4 class="opacity">Not available</h4>
              </div>
              <!-- Twitter Name -->
              <div v-if="user.twitter_username" class="links">
                ...
                <a v-bind:href="'https://twitter.com/'+ user.twitter_username" style="text-decoration: none;"><h4>{{ user.twitter_username }}</h4></a>
              </div>
              <!-- else Twitter Name -->
              <div v-else class="links">
                <svg class="opacity" height="18" width="20" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M20 2.799a8.549 8.549 0 01-2.363.647 4.077 4.077 0 001.804-2.266 8.194 8.194 0 01-2.6.993A4.099 4.099 0 009.75 4.977c0 .324.027.637.095.934-3.409-.166-6.425-1.8-8.452-4.288a4.128 4.128 0 00-.56 2.072c0 1.42.73 2.679 1.82 3.408A4.05 4.05 0 01.8 6.598v.045a4.119 4.119 0 003.285 4.028 4.092 4.092 0 01-1.075.135c-.263 0-.528-.015-.776-.07.531 1.624 2.038 2.818 3.831 2.857A8.239 8.239 0 01.981 15.34 7.68 7.68 0 010 15.285a11.543 11.543 0 006.29 1.84c7.545 0 11.67-6.25 11.67-11.667 0-.182-.006-.357-.015-.53A8.18 8.18 0 0020 2.798z"
                    fill="#fefefe"/>
                </svg>
            ...
            <div class="bottom-links">
              <!-- Github Link -->
              <div v-if="user.blog" class="links">
                <svg style="overflow: initial;" height="20" width="20" xmlns="http://www.w3.org/2000/svg">
                  <g fill="#fefefe">
                    <path
                ...
                <a v-bind:href="user.blog" style="text-decoration: none;"><h4 class="link-hover">{{ user.blog }}</h4></a>
              </div>
              <!-- else Github Link -->
              <div v-else class="links">
                <svg class="opacity" height="20" width="20" xmlns="http://www.w3.org/2000/svg">
                  <g fill="#fefefe">
                    <path
                      d="M7.404 5.012c-2.355 2.437-1.841 6.482.857 8.273.089.06.207.048.283-.027.568-.555 1.049-1.093 1.47-1.776a.213.213 0 00-.084-.3A2.743 2.743 0 018.878 10.1a2.64 2.64 0 01-.223-1.803c.168-.815 1.043-1.573 1.711-2.274l-.004-.002 2.504-2.555a2.568 2.568 0 013.648-.019 2.6 2.6 0 01.037 3.666l-1.517 1.56a.266.266 0 00-.06.273c.35 1.012.435 2.44.201 3.519-.006.03.031.05.053.028l3.228-3.295c2.062-2.105 2.044-5.531-.04-7.615a5.416 5.416 0 00-7.691.04L7.417 4.998l-.013.014z"/>
                    <path
                ...
                <h4 class="link-hover opacity">Not available</h4>
              </div>
              <!-- Company -->
              <div v-if="user.company" class="links">
                ...
                <a v-bind:href="'https://github.com/'+ user.company.slice(1)" style="text-decoration: none;"><h4>{{ user.company }}</h4></a>
              </div>
              <!-- else Company -->
              <div v-else class="links">
                <svg class="opacity" height="20" width="20" xmlns="http://www.w3.org/2000/svg">
